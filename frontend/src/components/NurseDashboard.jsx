...
import {
  patients, currentVitals, vitalRanges, initialAlerts, getRiskTrajectory, labInputFields, labResults,
} from '../mockData';
...
/* ── Mock nursing tasks ── */
const nursingTasks = [
  { id: 1, patient: 'Sarah Mitchell', bed: 'ICU-4A', task: 'Administer IV Meropenem 1g', time: '21:00', type: 'medication', priority: 'critical', done: false },
  { id: 2, patient: 'Ahmed Al-Rashid', bed: 'ICU-2B', task: 'Blood culture collection', time: '21:15', type: 'lab', priority: 'critical', done: false },
  { id: 3, patient: 'Sarah Mitchell', bed: 'ICU-4A', task: 'Fluid bolus — Normal Saline 500mL', time: '21:30', type: 'medication', priority: 'critical', done: false },
  { id: 4, patient: 'Maria Gonzalez', bed: 'ICU-6C', task: 'Record hourly vitals', time: '22:00', type: 'vitals', priority: 'high', done: false },
  { id: 5, patient: 'Fatima Hassan', bed: 'ICU-7A', task: 'Administer IV Piperacillin', time: '22:00', type: 'medication', priority: 'high', done: false },
  { id: 6, patient: 'James O\'Brien', bed: 'ICU-1A', task: 'Nebulizer treatment', time: '22:30', type: 'medication', priority: 'medium', done: false },
  { id: 7, patient: 'Yuki Tanaka', bed: 'ICU-3B', task: 'Post-op wound assessment', time: '23:00', type: 'assessment', priority: 'medium', done: false },
  { id: 8, patient: 'David Park', bed: 'ICU-8B', task: 'DVT prophylaxis injection', time: '23:00', type: 'medication', priority: 'medium', done: false },
  { id: 9, patient: 'Robert Williams', bed: 'ICU-5A', task: 'Discharge vitals & documentation', time: '23:30', type: 'vitals', priority: 'low', done: false },
];
...
/* ═══════════════ COMPONENT ═══════════════ */
export default function NurseDashboard({ notes = [], onAddNote }) {
  ...
  const handleUploadClick = () => {
    ...
    setTimeout(() => { clearInterval(interval); setUploadProgress(100); }, 3500);
  };
                      ...
                      {labResults[selectedPatient.id] && (
                        <div className="glass-card" style={{ marginBottom: 16, padding: 18 }}>
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 6 }}>
                            {labResults[selectedPatient.id].filter(l => l.status !== 'normal').slice(0, 6).map((lab, i) => (
                ...
                <div className="lab-form-grid">
                  {labInputFields.map(field => (
                    <div key={field.key} className="lab-input-group">
                      ...
                      <input type="number" step="any" placeholder={`e.g. ${field.range}`} />
                      <span className="lab-input-range">Reference: {field.range}</span>
                    </div>
                  ))}
                </div>
                <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 8 }}>
                  <button className="btn btn-primary">Submit Lab Results</button>
                </div>
              </div>
            ) : (
              <div className="glass-card" style={{ padding: 28 }}>
