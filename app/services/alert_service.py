...
# get_threshold not needed — DST v2 uses fixed alert tiers
...
def check_and_create_alert(db: Session, prediction: Prediction) -> Alert | None:
    ...
    Rules (DST v2 fixed tiers):
    - no_alert  : risk_score < 0.50  → no alert
    - high_risk : 0.50 <= score < 0.70
    - critical  : score >= 0.70
    - Prevent duplicate: don't create if unread alert already exists for this patient
    """
    # DST v2 uses fixed tiers: no_alert < 0.50, high_risk 0.50-0.70, critical >= 0.70
    ...
    alert_level = "critical" if prediction.risk_score >= 0.70 else "high_risk"

    alert = Alert(
        prediction_id=prediction.prediction_id,
        patient_id=prediction.patient_id,
        alert_message=f"⚠️ Sepsis alert for {patient_name}. "
                      f"Risk score: {prediction.risk_score:.2%} — "
                      f"Tier: {alert_level.upper().replace('_', ' ')}",
